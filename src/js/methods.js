...
function trapezoidolRuleMethod({ func, a, b, epsilon }) {
  let n = 10;
  let p = (mathFunction(func, a) + mathFunction(func, [b])) / 2, q;
  let h = (b - a) / n;
  for (let i = 0; i < n; i++) {
    x = a + i * h;
...
function simpson13RuleMethod(func, a, b, erro) {
  ...
  for (let i = 0; i < n; i += 2) {
    p += (h/3)*(mathFunction(func,[a+i*h]) + 4*mathFunction(func,[a+(i+1)*h]) + mathFunction(func,[a+(i+2)*h]));
  }
  do {
      q = p;
      n *= 2;
...
function simpson38RuleMethod({ func, a, b, epsilon }) {
  ...
  for (let i = 0; i < n; i += 3) {
    p += (3 / 8) * h * (mathFunction(func,[a + i * h]) + 3 * mathFunction(func,[a + (i + 1) * h]) + 3 * mathFunction(func,[a + (i + 2) * h]) + mathFunction(func,[a + (i + 3) * h]));
  }
  do {
    q = p;
    n *= 3;
...
function GaussQuadratureMethod(func, a, b) {
  let p = 0;
  for (let i = 0; i < 20; i++) {
    p += w[i] * (((b - a) / 2) * mathFunction(func, [a * ((1 - X[i]) / 2) + b * ((1 + X[i]) / 2)]));
  }
  return p;
}
